...
const e = /*@__PURE__*/ getDefaultExportFromCjs(
  /* @__PURE__ */ requirePicocolors(),
)
    ...
    Object.defineProperties(r, {
      ...
      ansi256ToAnsi: {
        value: (u) => {
          ...
          if (i === 0) return 30
          let D =
            30 + ((Math.round(s) << 2) | (Math.round(e) << 1) | Math.round(F))
          return i === 2 && (D += 60), D
        },
        enumerable: false,
      },
  ...
  for (const [E, a] of o.entries()) {
    if (((e += a), v.has(a))) {
      const { groups: B } = new RegExp(
        `(?:\\${W$1}(?<code>\\d+)m|\\${y}(?<uri>.*)${w$1})`,
      ).exec(o.slice(E).join("")) || { groups: {} }
      if (B.code !== void 0) {
...
var x = class {
  ...
  onKeypress(u, F) {
    if (
      ...
      F?.name &&
        (!this._track &&
          c.aliases.has(F.name) &&
          this.emit("cursor", c.aliases.get(F.name)),
...
function ce() {
  return g$1.platform !== "win32"
    ? g$1.env.TERM !== "linux"
    : !!g$1.env.CI ||
        !!g$1.env.WT_SESSION ||
        !!g$1.env.TERMINUS_SUBLIME ||
        g$1.env.ConEmuTask === "{cmd::Cmder}" ||
        g$1.env.TERM_PROGRAM === "Terminus-Sublime" ||
        g$1.env.TERM_PROGRAM === "vscode" ||
        g$1.env.TERM === "xterm-256color" ||
        g$1.env.TERM === "alacritty" ||
        g$1.env.TERMINAL_EMULATOR === "JetBrains-JediTerm"
}
const V = ce()
const u = (t, n) => (V ? t : n)
const le = u("❯", ">")
...
${e.gray(o)}`
        default:
          return `${r}${e.cyan(o)} ${
            this.value ? `${e.green(k)} ${n}` : `${e.dim(P)} ${e.dim(n)}`
          } ${e.dim("/")} ${
            this.value ? `${e.dim(P)} ${e.dim(s)}` : `${e.green(k)} ${s}`
          }
${e.cyan(d)}
`
      }
