...
import {
  S as version,
  x as description,
} from "./shared/bindingify-input-options-XPJLJOD0.mjs"
...
} from "./shared/rolldown-build-CtPvmZgJ.mjs"
...
var Command = class {
   ...
   */
  checkUnusedArgs() {
    const maximumArgsCount = this.args.some((arg) => arg.variadic)
      ? Infinity
      : this.args.length
  ...
  mri(argv, command) {
    ...
    for (const cliOption of cliOptions) {
      ...
      if (
        ...
        transforms[cliOption.name] === void 0
      ) {
...
function normalizeCliOptions(cliOptions, positionals) {
  ...
  if (options.configLoader !== void 0)
...
function camelizeNestedKeys(value) {
  ...
  for (const [key, nestedValue] of Object.entries(value))
    if (Array.isArray(nestedValue)) result[kebabToCamelCase(key)] = nestedValue
    else if (nestedValue && typeof nestedValue === "object")
      result[kebabToCamelCase(key)] = camelizeNestedKeys(nestedValue)
...
function parseCliArguments() {
  ...
  try {
    ...
    if (err?.name === "CACError") {
      ...
      if (match) {
        ...
        logger.error(
          `Option \`${optName}\` requires a value but none was provided.`,
        )
  ...
  const unknownKeys = Object.keys(parsedOptions).filter(
    (k) => !knownKeys.has(k),
  )
...
function _usingCtx() {
  var r =
      "function" == typeof SuppressedError
        ...
        : function (r, e) {
            var n = Error()
            return (
              (n.name = "SuppressedError"), (n.error = r), (n.suppressed = e), n
            )
          },
    e = {},
    n = []
  function using(r, e) {
    if (null != e) {
      ...
      if (
        void 0 === o &&
        ((o = e[Symbol.dispose || Symbol["for"]("Symbol.dispose")]), r)
      )
        var t = o
...
function printBundleOutputPretty(output) {
  ...
  printOutputEntries(
    outputEntries,
    collectOutputLayoutAdjustmentSizes(outputEntries),
    "<DIR>",
  )
}
...
if (!checkNodeVersion(g$1.versions.node))
  logger.warn(
    `You are using Node.js ${g$1.versions.node}. Rolldown requires Node.js version 20.19+ or 22.12+. Please upgrade your Node.js version.`,
  )
async function main() {
