...
var __reExport = (target, mod, secondTarget) => (
  ...
  secondTarget &&
    __copyProps(secondTarget, mod, "default")
)
var __toESM = (mod, isNodeMode, target) => (
  (target =
    mod !=
    null
      ? __create(__getProtoOf(mod))
      : {}),
  __copyProps(
...
function parseLoggingEnvironmentString(logEnvStr) {
  ...
  for (
    ...
    const option = args[i]
    const { value, extraPartCounter } = getEntireValue(
      i +
        1,
    )
    i += extraPartCounter
...
`
    ...
    write(s, _type) {
      if (
        ...
        import_fs.default.writeSync(this.fd, buf, 0, buf.length, null)
      }
  ...
  function extractWatchDirectoryCacheKey(path, currentDriveKey) {
    ...
    if (
      path.charCodeAt(1) ===
        typescript_exports.CharacterCodes.colon &&
      path.charCodeAt(2) ===
        typescript_exports.CharacterCodes.slash
    ) {
...
function createCancellationToken(args) {
  ...
  if (
    cancellationPipeName.charAt(
      cancellationPipeName.length -
        1,
    ) ===
    "*"
  ) {
