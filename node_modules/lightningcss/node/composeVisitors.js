...
function composeVisitors(visitors) {
  ...
  composeObjectVisitors(
    res,
    visitors,
    "Rule",
    ruleVisitor,
    wrapCustomAndUnknownAtRule,
  )
  composeObjectVisitors(
    res,
    visitors,
...
function composeObjectVisitors(res, visitors, key, apply, wrapKey) {
  ...
  let f = createArrayVisitor(visitors, (visitor, item) =>
    apply(visitor[key], item),
  )
  if (hasFunction) {
    res[key] = f
  } else {
  ...
  let v = createArrayVisitor(
    ...
    (visitor, /** @type {import('./ast').TokenOrValue} */ item) => {
      ...
      switch (item.type) {
        ...
        case "env":
          f = isExit
            ? visitor.EnvironmentVariableExit
            : visitor.EnvironmentVariable
          if (typeof f === "object") {
            let name
...
class Bitset {
  constructor(maxBits = 32) {
    this.bits = 0
    this.more =
      maxBits > 32 ? new Uint32Array(Math.ceil((maxBits - 32) / 32)) : null
  }
