...
/* eslint-disable no-var */ // second resume is a no-op
// concurrency changes are ignored while paused
// running jobs can't be killed
...
test("push with worker throwing error", function (t) {
  ...
  q.error(function (err, task) {
    t.ok(
      err instanceof
        Error,
      "global error handler should catch the error",
    )
  ...
  q.push(42, function (err) {
    t.ok(
      err instanceof
        Error,
      "push callback should catch the error",
    )
    t.match(err.message, /test error/, 'error message should be "test error"')
  })
})
...
test("unshift with worker throwing error", function (t) {
  ...
  q.unshift(42, function (err) {
    t.ok(
      err instanceof
        Error,
      "unshift callback should catch the error",
    )
    t.match(err.message, /test error/, 'error message should be "test error"')
  })
})
...
test("abort with error handler", function (t) {
  ...
  queue.error(function (err, task) {
    ...
    t.ok(
      task !==
        null,
      "error handler receives task value",
    )
    errorHandlerCalled++
  })
  queue.pause()
