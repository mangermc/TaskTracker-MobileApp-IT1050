  ...
  for (
    ...
    for (
      ...
      if (
        !char ||
        a !==
          char.repeat(a.length) ||
        b !==
          char.repeat(b.length)
      ) {
        continue
      }
...
const analyzeRepeatedExtglob = (body, options) => {
  ...
  const max =
    typeof options.maxExtglobRecursion ===
    "number"
      ? options.maxExtglobRecursion
  ...
  for (const branch of branches) {
    ...
    if (
      repeatedExtglobRecursion(branch) >
      max
    ) {
      return { risky: true }
    }
