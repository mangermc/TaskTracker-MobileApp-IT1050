...
test("address with @ in the name", function (t) {
  ...
  fxn = addrs.parseOneAddress
  result =
    fxn({ input: "ABC@abc (comment) < a@b.c>", atInDisplayName: true }) || {}
...
test("address with comma in the display name", function (t) {
  ...
  fxn = addrs.parseOneAddress
  result =
    fxn({ input: "ABC, abc (comment) <a@b.c>", commaInDisplayName: true }) || {}
  t.equal(result.name, "ABC, abc", "display name")

  result =
    fxn({ input: "ABC, abc (comment) <a@b.c>", commaInDisplayName: false }) ||
    {}
...
test("address semantic interpretation", function (t) {
  ...
  check("foo@  bar.com", "plain address with whitespace right of @ sign")
...
test("unicode support", function (t) {
  ...
  t.ok(
    result,
    "unicode support includes chinese characters (display-name, no quoted string)",
  )

  result = fxn('"杨孝宇" <xiaoyu@example.com>')
  t.ok(
    result,
    "unicode support includes chinese characters (display-name, quoted-string)",
  )
...
test("rejectTLD option", function (t) {
  ...
  t.notOk(
    result,
    "an address with a TLD for its domain is rejected when the option is set",
  )

  result = fxn({ input: '"Foo Bar" <foo@bar>', rejectTLD: false })
  t.ok(
    result,
    "a more complicated address with a TLD for its domain is allowed by rfc 5322",
  )

  result = fxn({ input: '"Foo Bar" <foo@bar>', rejectTLD: true })
  t.notOk(
    result,
    "a more complicated address with a TLD for its domain is rejected when the option is set",
  )
...
test("dots in unquoted display-names", function (t) {
  ...
  t.ok(
    result,
    "dots in the middle of an unquoted display-name with spaces (obs-phrase production)",
  )
  ...
  t.ok(
    result,
    "dots in the middle of an unquoted display-name without spaces (obs-phrase production)",
  )
...
test("rfc6854 - from", function (t) {
  ...
  t.equal(
    result[0].addresses[1].address,
    "carol@example.com",
    "Group address 1",
  )

  result = fxn(
    'Managing Partners:ben@example.com,carol@example.com;, "Foo" <foo@example.com>',
  )
...
test("rfc6854 - sender", function (t) {
  ...
  t.equal(
    ...
    "ben@example.com",
    "Result first address match",
  )
  t.equal(
    result.addresses[1].address,
    "carol@example.com",
    "Result first address match",
  )
...
test("rfc6854 - reply-to", function (t) {
  ...
  result = fxn(
    'Managing Partners:ben@example.com,carol@example.com;, "Foo" <foo@example.com>, Group2:alice@example.com;',
  )
...
test("whitespace in domain", function (t) {
  ...
  t.equal(
    result.domain,
    "Some-Group.Some-Org",
    "domain parsing strips whitespace",
  )
