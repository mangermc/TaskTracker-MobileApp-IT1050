...
const escapePosixPath = (path$5) =>
  path$5.replace(POSIX_UNESCAPED_GLOB_SYMBOLS, "\\$&")
const escapeWin32Path = (path$6) =>
  path$6.replace(WIN32_UNESCAPED_GLOB_SYMBOLS, "\\$&")
/**
...
function normalizePattern(pattern, opts, props, isIgnore) {
  ...
  result = (0, path.isAbsolute)(result.replace(ESCAPING_BACKSLASHES, ""))
    ? path.posix.relative(escapedCwd, result)
    : path.posix.normalize(result)
  const parentDir =
    (_PARENT_DIRECTORY$exe = PARENT_DIRECTORY.exec(result)) === null ||
    _PARENT_DIRECTORY$exe === void 0
      ? void 0
      : _PARENT_DIRECTORY$exe[0]
  ...
  if (!isIgnore && props.depthOffset >= 0) {
    ...
    props.root = ensureNonDriveRelativePath(
...
function processPatterns(options, patterns, props) {
  ...
  for (const pattern of patterns) {
    ...
    if (pattern[0] !== "!" || pattern[1] === "(")
      ...
      ignorePatterns.push(
        normalizePattern(pattern.slice(1), options, props, true),
      )
  }
  return {
    match: matchPatterns,
...
function formatPaths(paths, mapper) {
  if (mapper)
    for (let i = paths.length - 1; i >= 0; i--) paths[i] = mapper(paths[i])
  return paths
}
...
function getOptions(options) {
  ...
  opts.cwd = (
    opts.cwd instanceof URL
      ? (0, url.fileURLToPath)(opts.cwd)
      : (0, path.resolve)(opts.cwd || process.cwd())
  ).replace(BACKSLASHES, "/")
...
function getCrawler(globInput, inputOptions = {}) {
  ...
  if (
    globInput &&
    (inputOptions === null || inputOptions === void 0
      ? void 0
      : inputOptions.patterns)
  )
    throw new Error("Cannot pass patterns as both an argument and an option")
  const isModern = isReadonlyArray(globInput) || typeof globInput === "string"
  const patterns = ensureStringArray(
    (_ref = isModern ? globInput : globInput.patterns) !== null &&
      _ref !== void 0
      ? _ref
      : "**/*",
  )
