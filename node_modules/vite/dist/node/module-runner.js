...
const isAbsolute = function (p) {
  ...
  dirname = function (p) {
    let segments = normalizeWindowsPath(p)
      .replace(/\/$/, "")
      .split("/")
      .slice(0, -1)
    return (
  ...
  decodeBase64 = (() => {
    ...
    return capturedBuffer && typeof capturedBuffer.from == "function"
      ? (base64) => capturedBuffer.from(base64, "base64").toString("utf-8")
      : (base64) =>
          textDecoder.decode(
...
function encodePathChars(filepath) {
  return (
    ...
    filepath.includes("	") && (filepath = filepath.replace(tabRegEx, "%09")),
    filepath
  )
}
...
function posixPathToFileHref(posixPath) {
  ...
  return (
    ...
    resolved.includes("#") && (resolved = resolved.replace(hashRegex, "%23")),
...
var comma = 44,
  ...
  intToChar = /* @__PURE__ */ new Uint8Array(64),
  charToInt = /* @__PURE__ */ new Uint8Array(128)
...
function decodeInteger(reader, relative) {
  ...
  return (
    (value >>>= 1),
    shouldNegate && (value = -2147483648 | -value),
    relative + value
  )
}
...
function upperBound(haystack, needle, index) {
  for (
    let i = index + 1;
    i < haystack.length && haystack[i][COLUMN] === needle;
    index = i++
  );
  return index
}
...
function traceSegmentInternal(segments, memo, line, column, bias) {
  ...
  return (
    found
      ? (index = (bias === -1 ? upperBound : lowerBound)(
          segments,
          column,
          index,
        ))
...
var EvaluatedModuleNode = class {
  ...
  EvaluatedModules = class {
    ...
    ensureModule(id, url) {
      ...
      let fileModules =
        this.fileToModulesMap.get(moduleNode.file) || /* @__PURE__ */ new Set()
      return (
      ...
      return () => {
        try {
          ...
          for (let { deps, fn } of qualifiedCallbacks)
            fn(
              deps.map((dep) =>
                dep === acceptedPath ? fetchedModule : void 0,
              ),
            )
        ...
        (await new Promise((resolve, reject) => {
          socket.addEventListener(
            ...
            socket.addEventListener("close", () => {
              if (!isOpened) {
                reject(
                  /* @__PURE__ */ Error("WebSocket closed without opened."),
                )
                return
              }
              onMessage({
            ...
            await Promise.all(
              payload.updates.map(async (update) => {
                ...
                hmrClient.logger.error(
                  "css hmr is not supported in runner mode.",
                )
              }),
            ),
...
function retrieveFile(path) {
  ...
  return typeof content == "string"
    ? ((fileContentsCache[path] = content), content)
    : null
}
...
function retrieveSourceMap(source) {
  ...
  let sourceMapData
  return (
    reSourceMap.test(sourceMappingURL)
      ? ((sourceMapData = decodeBase64(
          sourceMappingURL.slice(sourceMappingURL.indexOf(",") + 1),
        )),
...
function CallSiteToString() {
  ...
  if (this.isNative()) fileLocation = "native"
    ...
    ;(fileName = this.getScriptNameOrSourceURL()),
      !fileName &&
        this.isEval() &&
        ((fileLocation = this.getEvalOrigin()), (fileLocation += ", ")),
  ...
  if (this.isToplevel() || isConstructor)
    ...
    functionName
      ? (typeName &&
          functionName.indexOf(typeName) !== 0 &&
          (line += `${typeName}.`),
        (line += functionName),
        methodName &&
...
function prepareStackTrace(error, stack) {
  ...
  return (
    ...
    errorString + processedStack.reverse().join("")
  )
}
//#endregion
...
function enableSourceMapSupport(runner) {
  if (runner.options.sourcemapInterceptor === "node") {
    ...
    return (
      ...
      () => !isEnabledAlready && process.setSourceMapsEnabled(!1)
    )
  }
...
var ModuleRunner = class {
  ...
  constructor(options, evaluator = new ESModulesEvaluator(), debug) {
    if (
      ...
      (this.debug = debug),
      (this.evaluatedModules =
        options.evaluatedModules ?? new EvaluatedModules()),
  ...
  async cachedRequest(url, mod, callstack = [], metadata) {
    ...
    if ((importee && mod.importers.add(importee), mod.evaluated && mod.promise))
    ...
    if (mod.promise)
      return mod.exports &&
        ...
        ? this.processImport(mod.exports, meta, metadata)
        : this.processImport(await mod.promise, meta, metadata)
    let debugTimer
    this.debug &&
      ...
      return (
        (this.builtinsPromise ??= (async () => {
          try {
            ...
            let builtins = (await this.transport.invoke("getBuiltins", [])).map(
              (builtin) =>
  ...
  async directRequest(url, mod, _callstack) {
    ...
    this.hmrClient &&
      Object.defineProperty(meta, "hot", {
        ...
        get: () => {
          if (!this.hmrClient)
            throw Error("[module runner] HMR client was closed.")
          return (
